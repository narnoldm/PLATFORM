...
I.  On Linux/Mac OS X
    ...
    B. Using make
        ...
        If you do not have cmake then you can use one of the supplied Makefiles.

        Supplied with this distribution are a set of makefiles named Makefile.ppp
        where ppp is a platform type.

        If a makefile exists with a "platform" similar to your platform then you 
        can build the library using:

            make BOOST_ROOT=iii -f mmm

        where:
            iii is the path to the boost include directory
            mmm is the Makefile corresponding to your platform.

     If none of the supplied makefiles work then you will have to modify one of
     the existing makefiles. If you do this please let us know about your changes
     so we can provide the customized makefile with our distribution.
